  ...
  const fetchSuggestions = useCallback(async (query) => {
    try {
      ...
      if (searchType === 'specialty') {
        ...
        const { data } = await api.get(`/api/doctors/specialties?q=${encodeURIComponent(query)}`)
        console.log(`✅ Specialties response:`, data)
        apiSuggestions = data.specialties || []
      } else if (searchType === 'location') {
        console.log(`📡 Calling: /api/doctors/locations?q=${encodeURIComponent(query)}`)
        const { data } = await api.get(`/api/doctors/locations?q=${encodeURIComponent(query)}`)
        console.log(`✅ Locations response:`, data)
        apiSuggestions = data.locations || []
      }
      
      // Sort suggestions by relevance
      const sortedSuggestions = apiSuggestions
        .filter(suggestion => 
